        ...
        <div class="hd-card form-card agendar-card">
          ...
          <div class="agendar-actions">
            <p id="agenda-selection" style="color:var(--hd-text-muted);font-size:.9rem;min-height:1.4rem">
              ${selectedDate && selectedTime ? `✅ ${selectedDate} a las ${selectedTime}` : 'Selecciona fecha y horario'}
            </p>
    ...
    <section class="section-pad" style="padding-top:0">
      <div class="section-inner">
        <div class="admin-metrics">
          <div class="hd-card admin-metric-card">
            <span class="hd-metric">${candidatos.length}</span>
            <span>Total candidatos</span>
          </div>
          <div class="hd-card admin-metric-card">
            <span class="hd-metric" style="color:var(--hd-cyan)">${byEstado.nuevo || 0}</span>
            <span>Nuevos</span>
          </div>
          <div class="hd-card admin-metric-card">
            <span class="hd-metric" style="color:#7c3aed">${byEstado.entrevista || 0}</span>
            <span>En entrevista</span>
          </div>
          <div class="hd-card admin-metric-card">
            <span class="hd-metric" style="color:#22c55e">${byEstado.aprobado || 0}</span>
            <span>Aprobados</span>
          </div>
          <div class="hd-card admin-metric-card">
            <span class="hd-metric" style="color:#0066ff">${byEstado.contratado || 0}</span>
            <span>Contratados</span>
          </div>
          <div class="hd-card admin-metric-card">
            <span class="hd-metric" style="color:#f59e0b">${entrevistas.length}</span>
            <span>Entrevistas agendadas</span>
          </div>
        </div>
        <div class="admin-nav-cards">
          <a href="#/admin/candidatos" class="admin-nav-card hd-card">
            <span style="font-size:2rem">👥</span>
            <strong>Candidatos</strong>
            <span>${candidatos.length} registros</span>
          </a>
          <a href="#/admin/entrevistas" class="admin-nav-card hd-card">
            <span style="font-size:2rem">📅</span>
            <strong>Entrevistas</strong>
            <span>${entrevistas.length} agendadas</span>
          </a>
        </div>
      </div>
    </section>
          ...
          ${list.map(c => {
            ...
            return `<tr data-id="${c.id}">
              <td>
                ...
                <span style="display:block;font-size:.8rem;color:var(--hd-text-muted)">${c.email}</span>
                <span style="display:block;font-size:.8rem;color:var(--hd-text-muted)">${c.tel}</span>
              </td>
              <td>${vLabel}</td>
              <td>
                <select class="estado-select hd-input" style="padding:4px 8px;font-size:.8rem;width:auto" data-id="${c.id}">
              ...
              <td>
                <button class="btn-view-cand hd-button-secondary" style="padding:4px 10px;font-size:.8rem" data-id="${c.id}">Ver</button>
                <button class="btn-del-cand" style="padding:4px 10px;font-size:.8rem;background:none;border:1px solid #ef4444;color:#ef4444;border-radius:8px;margin-left:4px;cursor:pointer" data-id="${c.id}">✕</button>
              </td>
            </tr>`;
          }).join('')}
        </tbody>
  ...
  function rebindTable() {
    document.querySelectorAll('.estado-select').forEach(sel => {
      sel.addEventListener('change', () => {
        updateCandidato(sel.dataset.id, { estado: sel.value });
      ...
      btn.addEventListener('click', () => {
        ...
        openModal(`
          ...
          <div class="step-summary">
            ...
            <div class="summary-row"><span>Área exp.</span><strong>${c.areaExp || '—'}</strong></div>
            <div class="summary-row"><span>Salario esp.</span><strong>${c.salarioEsp || '—'}</strong></div>
            <div class="summary-row"><span>Vacante</span><strong>${vObj ? vObj.titulo : c.vacante}</strong></div>
            <div class="summary-row"><span>Fuente</span><strong>${c.fuente || '—'}</strong></div>
            <div class="summary-row"><span>Inicio disp.</span><strong>${c.inicio || '—'}</strong></div>
            <div class="summary-row"><span>Estado</span><strong>${estadoBadge(c.estado)}</strong></div>
            ${c.mensaje ? `<div class="summary-row"><span>Mensaje</span><strong style="white-space:pre-wrap">${c.mensaje}</strong></div>` : ''}
          </div>
